...
14. Завершён `Deep smoke test`:
   - на production пройден сценарий `логин -> создание спора -> вход по коду -> первый аргумент -> архив -> возврат`;
   - подтверждено, что базовый dispute-flow после последних правок работает end-to-end;
