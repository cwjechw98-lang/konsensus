...
- 2026-03-17 — сделан второй проход по UX внутренних экранов Mini App: упрощены тексты `Споры / Открытые / События / Профиль / Новый спор / Ход / Учёба / Арена`, убраны служебные и англоязычные формулировки, облегчены contextual hints и поднята читаемость help-карточек.
...
| 2026-03-16 | Реализован `Telegram editorial reporting v1.2`: `release_announcements` расширен delivery-метриками (`00027`), publish-flow теперь пишет recipient/delivered/suppressed counts и `channel_message_id`, а в профиле для admin-пользователей появилась editorial-панель с последними delivery reports и очередью scheduled releases |
| 2026-03-16 | Локальная admin-whitelist синхронизирована с двумя адресами для ops-панелей (`KONSENSUS_ADMIN_EMAILS`), а `.env.example` уточнён примером формата списка email через запятую |
| 2026-03-16 | Реализован `Editorial Ops v1`: добавлены SQL-модели `editorial_release_cursor` и `editorial_release_drafts` (`00028`), AI-генерация release draft по GitHub compare range, отдельный admin-only маршрут `/ops/editorial`, publish/schedule через текущий Telegram release subsystem и перенос editorial delivery panel из профиля в ops-only слой |
| 2026-03-16 | Реализован `Editorial Ops v1.1`: `/ops` стал отдельным landing для admin-поверхностей, очередь ручной модерации апелляций вынесена в `/ops/appeals`, а профиль снова очищен от remaining admin-only блоков |
| 2026-03-16 | Реализован `Editorial Ops v1.2`: source layer для релизного draft generation обогащён `featureSignals`, `releaseTypeHint` и `userFacingScore`, AI-черновик учитывает эти сигналы при формулировке поста, а активный draft теперь можно rebased-ить на текущий `HEAD` без ручного пересоздания |
| 2026-03-16 | Реализован `Editorial Ops v1.3`: active draft показывает compare до/после последнего rebase, история draft-ов получила client-side фильтры `Все / Published / Cancelled`, а `/ops/editorial` теперь яснее подаёт статус workflow и scheduled state |
| 2026-03-16 | Реализован `Editorial Ops v1.4`: release workflow получил server-side типизацию сценариев (`Product update / UX refresh / Ops notice / Mixed release`), compare умеет переключаться между несколькими rebase-снимками, а history умеет искать drafts по title/slug/commit и отдельно фильтровать записи с rebase-историей |
| 2026-03-16 | Финализован `Editorial Ops v1`: active drafts получили bulk cancel, write-операции теперь режут stale-write по `updated_at`, release workflow type можно вручную корректировать прямо в draft editor, history поддерживает duplicate/reopen, а `/ops/editorial` показывает workflow timeline и delivery summary cards |
| 2026-03-17 | Срочный UX cleanup для Telegram Mini App: лендинг очищен от dev-терминов (`AI round package`, `Proof blocks`, `Public preview`, `Support bridge`), copy сокращён под mobile-скролл, support-strip больше не навязывается на каждом экране, добавлен scroll progress bar и улучшен safe-area для mobile bottom shell |
