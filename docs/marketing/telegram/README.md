...
- раздельной логики `канал = полный пост`, `бот = короткий тизер`;
- подавления bot-уведомления, если пользователь уже подписан на канал или группу.

Эти вещи пока оформлены как редакционный и продуктовый backlog, а не как готовая функция.
...
4. Если позже будет внедрена подписка-осведомлённая логика для бота, опираться на [delivery-model.md](/C:/project21/konsensus/docs/marketing/telegram/delivery-model.md).
